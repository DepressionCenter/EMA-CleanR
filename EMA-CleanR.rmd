...
However, when using the R code alone (without Markdown), the parameter values can be set below after each "else" statement.
```{r set-parameters}
### Input Parameters ###
...
# Indicates that input file has headers
input_file_has_headers <- if (exists("params") && !is.null(params$header)) params$header else TRUE

# Surveys completed before this time (typically 1 hour before the 1st AM survey) count towards the previous day
late_survey_cutoff_hour <- 
  if (exists("params") && !is.null(params$late_survey_cutoff_hour)) params$late_survey_cutoff_hour else 0 # default

# These survey names are ignored and removed from the data
ignore_surveys <-
...
# All EMA columns (one per question) must start with this prefix
ema_item_prefix <- 
  if (exists("params") && !is.null(params$ema_item_prefix)) params$ema_item_prefix else "EMA_" # default

# Total number of surveys. In this example, 4 surveys per day x 28 days = 112 surveys
total_surveys_in_study <-
  if (exists("params") && !is.null(params$total_surveys_in_study)) params$total_surveys_in_study else 112 # default
